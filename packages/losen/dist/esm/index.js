...
import require$$0$1, { createContext, forwardRef, useContext, useState, createElement, useEffect, useRef } from 'react';
import { connect } from 'react-redux';
import { bindActionCreators } from 'redux';
import require$$1 from 'react-dom';
import styled from 'styled-components';
import require$$1$1, { TextEncoder } from 'util';
...
const SHOW_RESET_MODAL = "SHOW_RESET_MODAL";
...
var isObject$d = /*@__PURE__*/getDefaultExportFromCjs(isObject_1);
...
function assignValue$3(object, key, value) {
  ...
  if (!(hasOwnProperty$d.call(object, key) && eq$5(objValue, value)) ||
...
var _baseSet = baseSet$3;

var baseSet$2 = _baseSet;
...
var setWith$1 = /*@__PURE__*/getDefaultExportFromCjs(setWith_1);
...
function vocalizeErrors(errors, nodeTitles) {
    if (errors.operator) {
        return errors.errors
            .map((error) => vocalizeErrors(error, nodeTitles))
...
function hasProperty(obj, prop) {
    return typeof get$2(obj, prop) !== "undefined";
}
...
function getValue(value, state) {
    if (typeof value === "object" && typeof value.fields !== "undefined") {
        return value.fields.reduce((acc, currentValue) => acc + get$2(state, currentValue), 0);
    }
    if (typeof value === "object" && value.field) {
        return get$2(state, value.field);
    }
    return value;
}
function buildGt({ field, value, errorMessage, }) {
    return (state) => {
        if (get$2(state, field) > getValue(value, state)) {
...
function buildLt({ field, value, errorMessage, }) {
    return (state) => {
        if (get$2(state, field) < getValue(value, state)) {
            return { valid: true, errors: [] };
        }
...
function buildGte({ field, value, errorMessage, }) {
    return (state) => {
        if (get$2(state, field) >= getValue(value, state)) {
            return { valid: true, errors: [] };
        }
...
function buildLte({ field, value, errorMessage, }) {
    return (state) => {
        if (get$2(state, field) <= getValue(value, state)) {
            return { valid: true, errors: [] };
        }
...
function buildEq({ field, value, errorMessage, }) {
    return (state) => {
        if (get$2(state, field) === getValue(value, state)) {
            return { valid: true, errors: [] };
        }
...
function buildNeq({ field, value, errorMessage, }) {
    return (state) => {
        if (get$2(state, field) !== getValue(value, state)) {
            return { valid: true, errors: [] };
        }
...
function buildBetween({ field, value, errorMessage, }) {
    return (state) => {
        const fieldValue = get$2(state, field);
...
function buildRequired({ field, errorMessage, }) {
    return (state) => {
        if (get$2(state, field) !== undefined) {
            return { valid: true, errors: [] };
        }
        if (errorMessage) {
            return { valid: false, errors: [errorMessage] };
...
function buildNot({ field, errorMessage, }) {
    return (state) => {
        if (!get$2(state, field)) {
            return { valid: true, errors: [] };
        }
        if (errorMessage) {
            return { valid: false, errors: [errorMessage] };
...
function buildIs({ field, errorMessage, }) {
    return (state) => {
        if (get$2(state, field, false)) {
            return { valid: true, errors: [] };
        }
        if (errorMessage) {
            return { valid: false, errors: [errorMessage] };
    ...
    return (state) => expression.clauses.reduce((res, clause) => {
        ...
        if (expression.errorMessage) {
            ...
            errors = validationResult.valid
                ? res.errors
                : Object.assign(Object.assign({}, res.errors), { errors: [
                        ...get$2(res.errors, "errors", []),
...
function translateNode(node, state, translations) {
    ...
    if (!get$2(currentTranslation, node.id)) {
        return node;
    }
    const nodeTranslation = get$2(currentTranslation, node.id);
    const result = Object.assign({}, node);
    if (nodeTranslation.heading) {
        set$1(result, "heading", nodeTranslation.heading);
    }
    if (nodeTranslation.description) {
        if (node.type === "Result" || node.type === "Page") {
            set$1(result, "lead", nodeTranslation.description);
        }
        else {
            set$1(result, "text", nodeTranslation.description);
        }
    }
    if (nodeTranslation.image) {
        set$1(result, "image", {
            url: nodeTranslation.image.large,
            alt: `Illustrasjon for ${get$2(result, "heading")}`,
        });
    }
    return result;
}
const mapTranslateNode = (state, translations) => (node) => translateNode(node, state, translations);
/**
 ...
 * result
 */
const parseTableCells = (state, translations) => (node) => {
    var _a, _b;
    if (!isOfType(node, ["Table"])) {
        return node;
    }
    return Object.assign(Object.assign({}, node), { cells: (_b = (_a = node.cells) === null || _a === void 0 ? void 0 : _a.map((row) => row === null || row === void 0 ? void 0 : row.map((cell) => {
            let inactive = false;
            if (cell.test) {
                inactive = !parseExpression(cell.test)(state[NAME]).valid;
            }
            const translatedProps = translateNode(cell, state, translations);
            return Object.assign(Object.assign(Object.assign({}, cell), translatedProps), { inactive });
        }))) !== null && _b !== void 0 ? _b : [] });
};
/**
 * Reduce option messages to a single (if any) message.
 */
const reduceOptionMessages = (state) => (messages) => (messages || [])
...
const reduceBranches = (state, nodeTitles, translations = {}, nodeMap) => (res, node) => {
    if (node.type !== "Branch") {
        return [...res, node];
    }
    // find a branch with a test that evaluates to true
    const selectedBranch = node.branches.find((branch) => parseExpression(branch.test)(state[NAME]).valid);
...
const mapWizardChildren = (state, nodeTitles, translations = {}, nodeMap) => (node) => {
    node.type;
    const currentValue = hasProperty(node, "property")
        ? getNodeValue(get$2(node, "property"), state)
        : undefined;
    ...
    if (isOfType(node, ["Input", "Number", "Textarea", "FetchOrg"]) &&
        node.validator &&
        currentValue) {
    ...
    if (hasProperty(node, "children")) {
        return Object.assign(Object.assign({}, translatedNode), (node.children && node.children.length
            ? {
                children: reduceWizard(node.children, state, nodeTitles, translations, nodeMap),
            }
            : {}));
    }
    return Object.assign(Object.assign(Object.assign({}, translatedNode), (currentValue !== undefined ? { currentValue } : {})), (!isOfType(node, ["Result"])
        ? {
            errors,
...
const reduceOptions = (state, translations, nodeMap) => (node) => {
    if (!hasProperty(node, "options")) {
        return node;
    }
    return Object.assign(Object.assign({}, node), { options: node.options
            ...
            .map((option) => {
            return Object.assign(Object.assign(Object.assign({}, option), (hasProperty(option, "disabled")
                ? {
...
const liftChildrenBranchPages = (res, node) => {
    ...
    if (hasProperty(node, "children") &&
        ((_b = node === null || node === void 0 ? void 0 : node.children) === null || _b === void 0 ? void 0 : _b[node.children.length - 1]) &&
...
const replaceReferences = (nodeMap) => (node) => {
    ...
    return Object.assign(Object.assign(Object.assign(Object.assign({}, omit$1(referencedNodeProps, ["show", "hide", "hidden"])), (hasProperty(node, "show") ? { show: node.show } : {})), (hasProperty(node, "hide") ? { hide: node.hide } : {})), (hasProperty(node, "hidden") ? { hidden: node.hidden } : {}));
};
// Build a flat object with all the nodes in the schema that have an ID
const buildNodeMap = (schema) => schema.reduce((res, node) => {
    var _a, _b;
    node.type;
    return Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({}, res), (hasProperty(node, "id") ? { [node.id]: node } : {})), (hasProperty(node, "children")
        ? buildNodeMap(node.children || [])
        : {})), (hasProperty(node, "options") ? buildNodeMap(node.options || []) : {})), (isOfType(node, ["Branch"])
        ? buildNodeMap((_b = (_a = node === null || node === void 0 ? void 0 : node.branches) === null || _a === void 0 ? void 0 : _a.reduce((children, branch) => [...children, ...branch.children], [])) !== null && _b !== void 0 ? _b : [])
        : {}));
}, {});
/**
 ...
 */
const discardVisibilityProps = (node) => omit$1(node, ["show", "hide", "hidden"]);
...
function processComputed(schema, state) {
    return Object.keys(schema.computed || {}).reduce((res, prop) => {
        var _a;
        if (typeof ((_a = schema === null || schema === void 0 ? void 0 : schema.computed) === null || _a === void 0 ? void 0 : _a[prop]) === "undefined") {
            return res;
        }
        return Object.assign(Object.assign({}, res), { [prop]: parseExpression(schema.computed[prop])(state).valid });
    }, {});
}

const initialState = {};
const removeInactiveQuestions = (state, nodes, path = []) => Object.keys(state).reduce((acc, property) => {
    // page, $computed and $external properties we want to keep
    if (["page", "$computed", "$external", "$language"].indexOf(property) > -1) {
        return Object.assign(Object.assign({}, acc), { [property]: state[property] });
    }
    ...
    if (find$1(nodes, (node) => hasProperty(node, "property") && node.property === fullPropertyPath)) {
        return Object.assign(Object.assign({}, acc), { [property]: state[property] });
    }
    // recurse if the property is an object
    if (isObject$d(state[property])) {
        return Object.assign(Object.assign({}, acc), { 
            // remove inactive properties on the property object
            [property]: removeInactiveQuestions(state[property], // pass the object we got to
            nodes, [...path, property] // add the property to the path and go on
            ) });
    }
...
function applyComputed(wizard, state) {
    return Object.assign(Object.assign({}, state), { $computed: processComputed(wizard, state) });
}
...
function setDataUpdate(wizard, state, { payload }) {
    ...
    const newState = setWith$1(Object.assign({}, state), payload.key, payload.value, (nsValue) => nsValue || {});
...
var reducer = (wizard) => (state = applyComputed(wizard, initialState), action) => {
    const { type } = action;
    switch (type) {
        case SET_DATA:
        ...
        case SHOW_RESET_MODAL:
            return Object.assign(Object.assign({}, state), { $showResetModal: action.payload.show });
        default:
            return state;
    }
};
...
Object.defineProperty(Modal$3, "__esModule", {
  value: true
});
Modal$3.bodyOpenClassName = Modal$3.portalClassName = undefined;
...
var portalClassName = Modal$3.portalClassName = "ReactModalPortal";
var bodyOpenClassName = Modal$3.bodyOpenClassName = "ReactModal__Body--open";
...
var Modal$2 = function (_Component) {
...
(0, _reactLifecyclesCompat.polyfill)(Modal$2);

if (process.env.NODE_ENV !== "production") {
  Modal$2.setCreateHTMLElement = function (fn) {
    return createHTMLElement = fn;
  };
}

Modal$3.default = Modal$2;
...
function StyleProvider({ children, styles: styles$1 = {} }) {
    return jsx(StyleContext.Provider, Object.assign({ value: merge$2({}, styles, styles$1) }, { children: jsx("div", { children: children }) }));
}

const injectStyles = (WrappedComponent) => forwardRef((props, ref) => {
    const styles = useContext(StyleContext);
    return (jsx(WrappedComponent, Object.assign({}, props, { ref: ref, styles: styles })));
});

const H1 = injectStyles(styled.h1 `
...
`);
const H2 = injectStyles(styled.h2 `
...
`);
const H3 = injectStyles(styled.h3 `
  font-family: ${({ styles }) => styles.font.primary};
  font-size: 18px;
  font-style: normal;
  font-weight: bold;
...
`);
const H4 = injectStyles(styled.h4 `
  font-family: ${({ styles }) => styles.font.primary};
  font-size: 14px;
  text-transform: uppercase;
  letter-spacing: 1px;
...
const P = injectStyles(styled.p `
  font-size: inherit;
  font-weight: 300;
  margin: 0 0 24px;
  max-width: 600px;
`);
const Lead = injectStyles(styled.div `
  font-family: ${({ styles }) => styles.font.secondary};
  font-weight: 300;
  font-size: 22px;
  line-height: 1.4;
...
var Paragraphs = /*#__PURE__*/Object.freeze({
    __proto__: null,
    Lead: Lead,
    P: P
});

const MainButton = injectStyles(styled.button `
  font-family: ${({ styles }) => styles.font.primary};
  font-weight: normal;
  font-size: 18px;
  color: white;
...
`);
const SecondaryButton = injectStyles(styled.button `
  font-family: ${({ styles }) => styles.font.primary};
  font-weight: normal;
  font-size: 18px;
...
`);
const SmallButton = injectStyles(styled.button `
  font-family: ${({ styles }) => styles.font.primary};
  font-size: 16px;
  color: white;
  background: ${({ styles }) => styles.color.light};
...
`);
const NavButton$1 = injectStyles(styled.button `
  font-family: ${({ styles }) => styles.font.primary};
  font-weight: normal;
  font-size: 18px;
  color: white;
...
function Modal(props) {
    const [showModal, setShowModal] = require$$0$1.useState(true);
    ...
    return (jsx(ReactModal, Object.assign({ aria: {
            labelledby: "heading",
            describedby: "full_description",
        }, ariaHideApp: false, isOpen: showModal && Object.keys(props.data).length !== 0, role: "dialog", shouldCloseOnEsc: true, onRequestClose: handleCloseModal, style: customStyle }, { children: jsxs(ModalBox, { children: [jsx(H1, Object.assign({ id: "heading" }, { children: "Vil du starte p\u00E5 nytt?" })), jsx(Lead, Object.assign({ id: "full_description" }, { children: "Veiviseren husker svarene fra ditt forrige bes\u00F8k." })), jsx(MainButton, Object.assign({ type: "button", onClick: handleCloseModal }, { children: "Fortsett" })), jsx("br", {}), jsx(SecondaryButton, Object.assign({ type: "button", onClick: handleRestart }, { children: "Start p\u00E5 nytt" }))] }) })));
}
var Modal$1 = connect((state) => ({ data: state[NAME] }), (dispatch) => bindActionCreators({ resetData }, dispatch))(Modal);

function getNodeErrors(node) {
    const errors = Object.assign({ validation: {}, disabled: [] }, node.errors);
    return ((Array.isArray(errors.disabled) ? errors.disabled.length : 0) +
        (errors.required ? 1 : 0) +
    ...
    return pages
        ...
        return [
            ...res,
            {
                ...
                type,
                heading,
                errorNodes: (_b = (_a = children === null || children === void 0 ? void 0 : children.filter((node) => hasProperty(node, "property"))) === null || _a === void 0 ? void 0 : _a.filter((node) => getNodeErrors(node) > 0)) === null || _b === void 0 ? void 0 : _b.map((node) => ({
                    property: get$2(node, "property"),
                    heading: get$2(node, "heading"),
                    errors: get$2(node, "errors"),
                })),
            },
        ];
    }, [])
        .filter(({ errorNodes }) => errorNodes === null || errorNodes === void 0 ? void 0 : errorNodes.length);
}
function getTitle(state, { meta: { title } }, translations) {
    ...
    return get$2(translations, [currentLanguage, "meta", "title"], title);
}
function getNodeTitles(schema, translations, state) {
    const translation = getTranslation(state, translations);
    return schema.reduce((res, node) => (Object.assign(Object.assign(Object.assign(Object.assign({}, res), (hasProperty(node, "id")
        ? {
            [node.id]: (node.id && get$2(translation, `['${node.id}'].heading`)) ||
                get$2(node, "heading", node.id),
        }
        : {})), (hasProperty(node, "children")
        ? getNodeTitles(node.children || [], translations, state)
        : {})), (hasProperty(node, "branches")
        ? node.branches.reduce((branchesRes, branch) => (Object.assign(Object.assign({}, branchesRes), getNodeTitles(branch.children, translations, state))), {})
        : {}))), {});
}
...
function getPages(reducedSchema, nodeTitles) {
    return reducedSchema.map((node) => {
        const errors = hasProperty(node, "children")
            ...
            : 0;
        return Object.assign(Object.assign({}, node), { errors, heading: get$2(nodeTitles, node.id, hasProperty(node, "heading") ? node.heading : undefined), completed: !errors });
    });
}

const NavResult$1 = injectStyles(styled.div `
  font-family: ${({ styles }) => { var _a; return (_a = styles === null || styles === void 0 ? void 0 : styles.font) === null || _a === void 0 ? void 0 : _a.secondary; }};
  font-size: 1em;
  font-weight: normal;
  text-align: center;
  color: white;
  background: ${({ styles }) => { var _a; return (_a = styles === null || styles === void 0 ? void 0 : styles.color) === null || _a === void 0 ? void 0 : _a.light; }};
  width: 100%;
  ...
  appearance: none;
  margin-top: 30px;
  margin-right: ${(props) => ((props === null || props === void 0 ? void 0 : props.next) ? "0" : "auto")};
  margin-left: ${(props) => ((props === null || props === void 0 ? void 0 : props.next) ? "auto" : "0")};
  margin-bottom: 10px;
  position: relative;
  transition: background 0.1s ease-in-out;
  ...
  &:hover {
    background: ${({ styles }) => styles === null || styles === void 0 ? void 0 : styles.color.dark};
  }
`);
const NavReset$2 = injectStyles(styled.button `
  font-family: ${({ styles }) => styles === null || styles === void 0 ? void 0 : styles.font.secondary};
  font-size: 1em;
  font-weight: normal;
  text-align: center;
  color: ${({ styles }) => styles === null || styles === void 0 ? void 0 : styles.color.light};
  background: white;
  width: 100%;
  padding: 12px 18px 13px;
  cursor: pointer;
  user-select: none;
  border: 1px solid ${({ styles }) => styles === null || styles === void 0 ? void 0 : styles.color.warmgray2};
  border-radius: 0;
  appearance: none;
  margin-right: ${(props) => (props.next ? "0" : "auto")};
  margin-left: ${(props) => (props.next ? "auto" : "0")};
  ...
  text-rendering: optimizeLegibility;
  &:hover {
    background: ${({ styles }) => styles === null || styles === void 0 ? void 0 : styles.color.warmgray2};
  }
`);

var NavButton = /*#__PURE__*/Object.freeze({
    __proto__: null,
    NavReset: NavReset$2,
    NavResult: NavResult$1
});

const NavItem$1 = injectStyles(styled.a `
...
`);
const TextBlock = injectStyles(styled.div `
  padding: 30px 0 10px;
  ${(props) => props.groupedSimple
    ? ""
    : `
...
const ErrorMessage = injectStyles(styled.p `
...
`);
const ErrorMessageAlt = injectStyles(styled.p `
  font-family: ${({ styles }) => styles.font.secondary};
`);
...
var Figure$1 = /*#__PURE__*/Object.freeze({
    __proto__: null,
    default: Figure
});

const Nav$1 = injectStyles(styled.nav `
...
const Navigation$1 = injectStyles(styled.main `
  display: flex;
  margin-bottom: 50px;
  max-width: ${({ styles }) => styles.size.blockWidth};
`);
...
const Checkbox$1 = injectStyles(styled.input `
  position: absolute;
  opacity: 0;
  z-index: -1;
  line-height: 1.4;
...
`);
const CheckboxResultDisplay = injectStyles(styled.input `
  position: absolute;
  opacity: 0;
  z-index: -1;
  line-height: 1.4;
...
`);
const Radio$1 = injectStyles(styled.input.attrs({
    type: "radio",
}) `
  position: absolute;
  opacity: 0;
...
`);
const NumberInput = injectStyles(styled.input `
  font-family: inherit;
  font-size: 1em;
  text-align: right;
  padding: 0.5em;
...
`);
const TextInput = injectStyles(styled.input `
  font-family: inherit;
  font-size: 1em;
  padding: 0.5em;
  width: auto;
  min-width: 40%;
  border: 1px solid
    ${({ styles, validation: { error } = {} }) => error ? styles.color.red : styles.color.darkgray} !important;
  &::placeholder {
    font-color: inherit;
    opacity: 0.6;
  }
...
`);
const Textarea$1 = injectStyles(styled.textarea `
  font-family: inherit;
  font-size: 1em;
  padding: 0.5em;
  width: 100%;
...
`);
const SelectWrapper = injectStyles(styled.div `
  position: relative;
  width: 100%;
  max-width: 400px;
  &::before {
...
const Label = injectStyles(styled.label `
  ...
  > div {
    ...
    : ""};
`);
const SRLabel = injectStyles(styled.label `
  position: absolute;
  width: 1px;
  height: 1px;
  padding: 0;
...
const ToggleButton = injectStyles(styled.button `
  width: ${({ styles }) => styles.size.mobileContentWidth};
  margin: 2px auto;
  background: white;
...
const Wizard$2 = injectStyles(styled.div `
...
function NavItem({ active = false, done = false, heading = "TITLE MISSING", id, index, setPage }) {
    return (jsxs(NavItem$1, Object.assign({ active: active, done: done, href: "#", onClick: (e) => {
            e.preventDefault();
            setPage(id);
        } }, { children: [jsx("div", { children: index }), jsxs("p", { children: [heading, active ? "" : jsx("span", { children: "Endre svar" })] })] })));
}

function NavReset(props) {
    return (jsx(SecondaryButton, Object.assign({ type: "button", onClick: () => {
            /* eslint no-alert: 0 */
            if (confirm('Sikker på at du vil slette alle svarene dine?')) {
                props.resetData();
                props.showIntro();
            }
        } }, { children: "Start p\u00E5 nytt" })));
}
var NavReset$1 = connect(null, { resetData })(NavReset);

function NavResult({ setPage, id, active = false }) {
    return (jsx(MainButton, Object.assign({ type: "button", "data-id": id, active: active, onClick: () => setPage(id) }, { children: "Vis resultat" })));
}
...
function LanguageSelector({ translations: translationsObject, currentLanguage, setLanguage }) {
    const handleLanguageSelect = (languageKey) => () => setLanguage(languageKey);
    // Get an array of the translations from the object values
    const translations = values$1(translationsObject);
    ...
    return (jsx(Simple, Object.assign({ padItems: true }, { children: [
            /**
            ...
            ...translations,
        ].map(({ key, name }) => (jsx(SmallButton, Object.assign({ disabled: key === currentLanguage, onClick: handleLanguageSelect(key) }, { children: name }), key))) })));
}
const mapStateToProps$3 = (state) => ({
    currentLanguage: getCurrentLanguage(state),
});
var LanguageSelector$1 = connect(mapStateToProps$3, { setLanguage })(LanguageSelector);

function Nav({ heading = "Missing page heading", page: currentPage, setPage, tableOfContents, showIntro, translations }) {
    const [tocExpanded, setTocExpanded] = useState(true);
    const toggleToc = () => setTocExpanded(!tocExpanded);
    return (jsxs("div", { children: [jsxs(ToggleButton, Object.assign({ type: "button", onClick: toggleToc }, { children: [tocExpanded ? "Vis" : "Skjul", " alle steg"] })), jsxs(Nav$1, Object.assign({ tocExpanded: tocExpanded }, { children: [jsx(Title, Object.assign({ onClick: showIntro }, { children: heading })), tableOfContents.map((page, index) => page.type === "Result" ? (jsx(NavResult, { id: page.id, setPage: setPage, 
                        // TODO: These 👇 were passed, but wasn't being received by NavResult..
                        // heading={page.heading}
                        // done={!page.errors}
                        active: page.id === currentPage }, page.id)) : (jsx(NavItem, { id: page.id, index: index + 1, heading: page.heading, setPage: setPage, done: !page.errors, active: page.id === currentPage }, page.id))), jsx(NavReset$1, { showIntro: showIntro }), jsx(LanguageSelector$1, { translations: translations })] }))] }));
}
...
const Fieldset = injectStyles(styled.fieldset `
  border: 0;
  padding: 0;
  margin: 0;
`);
...
function Html({ id, text, inline, h2, h3 }) {
    ...
    if (h2) {
        ...
        return (jsx(H2, { id: id, inline: inline, dangerouslySetInnerHTML: { __html: text } }));
    }
    if (h3) {
        // eslint-disable-next-line react/no-danger
        return (jsx(H3, { id: id, inline: inline, dangerouslySetInnerHTML: { __html: text } }));
    }
    // eslint-disable-next-line react/no-danger
    return (jsx(StyledHtml, { id: id, inline: inline, dangerouslySetInnerHTML: { __html: text } }));
}

function CheckboxInput(_a) {
    var { checked, debug, disabled, heading, id, image, messages, name, onChange, text } = _a; __rest(_a, ["checked", "debug", "disabled", "heading", "id", "image", "messages", "name", "onChange", "text"]);
    const message = messages === null || messages === void 0 ? void 0 : messages[0];
    return (jsxs("div", { children: [jsx(Checkbox$1, { "aria-label": heading, type: "checkbox", name: name, id: id, onChange: onChange, disabled: disabled, checked: checked }), jsxs(Label, Object.assign({ htmlFor: id, warning: message === null || message === void 0 ? void 0 : message.warning, debug: debug }, { children: [jsxs("div", Object.assign({ "data-id": id }, { children: [jsxs("div", { children: [heading && !text ? heading : null, heading && text ? jsx(H3, { children: heading }) : null] }), text ? jsx(Html, { text: text }) : null] })), image && image.url ? jsx("img", { src: image.url, alt: image.alt }) : null] }))] }));
}

function Checkbox({ setData, heading, options, grid, disabled, property, debug, currentValue, data, update }) {
    const handleChange = (property, value) => (e) => {
        setData(`${property}.${value}`, e.target.checked);
        // Unset properties in the update array when the checkbox value change
        update === null || update === void 0 ? void 0 : update.forEach((updateProp) => {
            setData(updateProp, undefined, true);
        });
    };
    return (jsxs(Fieldset, { children: [jsx(SRLegend, { children: heading }), options && options.length ? (jsx(OptionWrapper, Object.assign({ grid: grid }, { children: options.map((option) => {
                    const isDisabled = disabled || option.disabled;
                    return (createElement(CheckboxInput, Object.assign({}, option, { data: data, disabled: isDisabled, key: `${property}:${option.value}`, debug: debug, checked: currentValue === null || currentValue === void 0 ? void 0 : currentValue[option.value], name: property, onChange: !isDisabled ? handleChange(property, option.value) : undefined })));
                }) }))) : null] }));
}

function ErrorIcon({ textFill = 'white', triangleFill = styles.color.red }) {
    let label = 'Validaringsfeil';
    if (triangleFill === 'black') {
        label = 'Informasjon';
    }
    return (jsx("svg", Object.assign({ xmlns: "http://www.w3.org/2000/svg", viewBox: "0 0 38 38", "aria-label": label }, { children: jsxs("g", { children: [jsx("circle", { style: { fill: triangleFill }, cx: "19", cy: "19", r: "16" }), jsx("path", { style: { fill: textFill }, d: "M16.9,25.1c0-0.7,0.2-1.2,0.5-1.5s0.9-0.5,1.5-0.5c0.6,0,1.2,0.2,1.5,0.5s0.5,0.8,0.5,1.5\n            c0,0.6-0.2,1.1-0.5,1.5s-0.9,0.5-1.5,0.5c-0.7,0-1.2-0.2-1.5-0.5S16.9,25.7,16.9,25.1z M20.4,21.1h-2.9L17,9.6H21L20.4,21.1z" })] }) })));
}
...
function Evaluation({ data, testing, final, groupedSimple, happy, sad, showValue, unit, }) {
    let testedValue = 0;
    if (Array.isArray(testing)) {
        testedValue = get$2(data, testing[0]) - get$2(data, testing[1]);
    }
    else if (testing) {
        testedValue = get$2(data, testing);
    }
    const absTestedValue = Math.abs(testedValue);
    if (showValue) {
        if (testedValue < 0) {
            return (jsx(StyledSum, Object.assign({ groupedSimple: groupedSimple, final: final }, { children: jsxs("div", { children: [sad, jsxs("span", Object.assign({ className: "sad" }, { children: [absTestedValue, " ", unit ? jsx(Html, { inline: true, text: unit }) : null] }))] }) })));
        }
        return (jsx(StyledSum, Object.assign({ groupedSimple: groupedSimple, final: final }, { children: jsxs("div", { children: [happy, jsxs("span", Object.assign({ className: "happy" }, { children: [testedValue, " ", unit ? jsx(Html, { inline: true, text: unit }) : null] }))] }) })));
    }
    if (testedValue < 0) {
        // sad case
        return (jsx(StyledSumResult, { children: jsx("div", Object.assign({ className: "sad" }, { children: sad })) }));
    }
    // happy case
    return (jsx(StyledSumResult, { children: jsx("div", Object.assign({ className: "happy" }, { children: happy })) }));
}

const Table$1 = injectStyles(styled.table `
  width: 100%;
  font-size: 14px;
  border: 1px solid ${({ styles }) => styles.color.darkgray};
  border-collapse: collapse;
...
`);
const TD = injectStyles(styled.td `
...
function ApprovalAreas({ areas, title }) {
    ...
    let heading;
    if (title) {
        heading = jsx(H3, { children: "Sentrale godkjenninger" });
    }
    return (jsxs("div", { children: [heading, jsx(TableContainer, { children: jsxs(Table$1, { children: [jsx("thead", { children: jsxs("tr", { children: [jsx("th", { children: "Funksjon" }), jsx("th", { children: "Fagomr\u00E5de" }), jsx("th", { children: "Tiltaksklasse" })] }) }), jsx("tbody", { children: areas.map((area, rowKey) => (jsxs("tr", { children: [jsx("td", { children: area.function }), jsx("td", { children: area.subject_area }), jsx("td", { children: area.grade })] }, rowKey))) })] }) })] }));
}

function Input({ heading, property, errors, disabled, currentValue, placeholder, autocomplete, inputType, setData, update, }) {
    var _a, _b, _c;
    const handleChange = (e) => {
        setData(property, e.target.value);
        update === null || update === void 0 ? void 0 : update(e.target.value);
    };
    return (jsxs("div", { children: [jsx(TextInput, { "aria-label": heading, "aria-invalid": (_a = errors === null || errors === void 0 ? void 0 : errors.validation) === null || _a === void 0 ? void 0 : _a.error, "aria-describedby": ((_b = errors === null || errors === void 0 ? void 0 : errors.validation) === null || _b === void 0 ? void 0 : _b.error) ? `${property}.error` : undefined, disabled: disabled, id: property, onChange: handleChange, placeholder: placeholder, autoComplete: autocomplete, type: inputType, validation: errors === null || errors === void 0 ? void 0 : errors.validation, value: currentValue }), ((_c = errors === null || errors === void 0 ? void 0 : errors.validation) === null || _c === void 0 ? void 0 : _c.error) && (jsxs(ErrorMessage, Object.assign({ role: "alert", id: `${property}.error` }, { children: [jsx(ErrorIcon, {}), " ", errors.validation.message] })))] }));
}
...
function VariableText({ data = {}, text = "" }) {
    ...
    const compiled = template$1(text);
    // eslint-disable-next-line react/no-danger
    return jsx("div", { dangerouslySetInnerHTML: { __html: compiled(data) } });
}
...
const Information$2 = injectStyles(styled.div `
  svg {
    width: 1.2em;
    height: 1.2em;
    display: inline-block;
...
function FetchOrd(props) {
    ...
    const { information, invalidapproval, invalidOrg, property, setData, currentValue, SGheading, source = 'https://data.brreg.no/enhetsregisteret/api/enheter/', SGtext, SGsource = 'https://sgregister.dibk.no/api/enterprises/' } = props;
    const fetchOrgData = (orgid) => __awaiter(this, void 0, void 0, function* () {
        ...
        try {
            ...
            merge$2(updateData, {
                orgid: orgid,
                name: (result === null || result === void 0 ? void 0 : result.navn) || '',
                dataOrg: true,
                invalidOrg: false,
            });
            if ((_f = result === null || result === void 0 ? void 0 : result.postadresse) === null || _f === void 0 ? void 0 : _f.adresse) {
                merge$2(updateData, {
                    postcode: ((_g = result === null || result === void 0 ? void 0 : result.postadresse) === null || _g === void 0 ? void 0 : _g.postnummer) || '',
                    postplace: ((_h = result === null || result === void 0 ? void 0 : result.postadresse) === null || _h === void 0 ? void 0 : _h.poststed) || '',
                    address: ((_j = result === null || result === void 0 ? void 0 : result.postadresse) === null || _j === void 0 ? void 0 : _j.adresse) || '',
                });
            }
            else {
                merge$2(updateData, {
                    postcode: ((_k = result === null || result === void 0 ? void 0 : result.forretningsadresse) === null || _k === void 0 ? void 0 : _k.postnummer) || '',
                    postplace: ((_l = result === null || result === void 0 ? void 0 : result.forretningsadresse) === null || _l === void 0 ? void 0 : _l.poststed) || '',
                    address: ((_m = result === null || result === void 0 ? void 0 : result.forretningsadresse) === null || _m === void 0 ? void 0 : _m.adresse) || '',
                });
            }
        }
        catch (error) {
        ...
        try {
            ...
            const status = (_p = (_o = result['dibk-sgdata']) === null || _o === void 0 ? void 0 : _o.status) !== null && _p !== void 0 ? _p : false;
            const validApprovalAreas = (_r = (_q = result === null || result === void 0 ? void 0 : result['dibk-sgdata']) === null || _q === void 0 ? void 0 : _q.valid_approval_areas) !== null && _r !== void 0 ? _r : [];
            merge$2(updateData, {
                status,
                validApprovalAreas,
                dataSG: true
        ...
        setData(property, Object.assign(Object.assign({}, currentValue), updateData));
        setData('sgdata', (_t = (_s = updateData === null || updateData === void 0 ? void 0 : updateData.status) === null || _s === void 0 ? void 0 : _s.approved) !== null && _t !== void 0 ? _t : false, true);
        setLoading(false);
    });
    const update = (value) => {
    ...
    return (jsxs("div", { children: [jsx(Input, Object.assign({}, props, { type: "Input", currentValue: currentValue === null || currentValue === void 0 ? void 0 : currentValue.orgid, setData: (property, value) => setData(property, { orgid: value }), update: update })), (currentValue === null || currentValue === void 0 ? void 0 : currentValue.dataOrg) ? (jsxs("div", { children: [jsx("br", {}), jsxs(DL, Object.assign({ role: "status" }, { children: [jsx("dt", { children: "Firmaets navn" }), jsx("dd", { children: (_a = currentValue === null || currentValue === void 0 ? void 0 : currentValue.name) !== null && _a !== void 0 ? _a : '' }), jsx("dt", { children: "Adresse" }), jsx("dd", { children: (_b = currentValue === null || currentValue === void 0 ? void 0 : currentValue.address) !== null && _b !== void 0 ? _b : '' }), jsx("dt", { children: "Postnummer- og sted" }), jsxs("dd", { children: [(_c = currentValue === null || currentValue === void 0 ? void 0 : currentValue.postcode) !== null && _c !== void 0 ? _c : '', ' ', (_d = currentValue === null || currentValue === void 0 ? void 0 : currentValue.postplace) !== null && _d !== void 0 ? _d : ''] })] })), jsxs(Information$2, { children: [jsx(ErrorIcon, { triangleFill: 'black' }), jsx(Html, { text: information })] })] })) : null, jsx("div", { children: jsxs("div", { children: [loading && jsx(Loading, Object.assign({ role: "status" }, { children: "Laster inn data" })), !loading && (currentValue === null || currentValue === void 0 ? void 0 : currentValue.dataSG) ? (jsxs(Notice, { children: [jsx(H3, { children: jsx(VariableText, { text: SGheading, data: currentValue }) }), jsx(ApprovalAreas, { areas: (_e = currentValue === null || currentValue === void 0 ? void 0 : currentValue.validApprovalAreas) !== null && _e !== void 0 ? _e : [] }), jsx(VariableText, { text: SGtext, data: currentValue })] })) : null, !loading && !(currentValue === null || currentValue === void 0 ? void 0 : currentValue.dataSG) && (currentValue === null || currentValue === void 0 ? void 0 : currentValue.orgid) ? jsx(H3, { children: invalidapproval }) : null] }) }), jsx("div", { children: (currentValue === null || currentValue === void 0 ? void 0 : currentValue.invalidOrg) ? (jsxs(ErrorMessage, Object.assign({ role: "alert" }, { children: [jsx(ErrorIcon, {}), " ", invalidOrg] }))) : null })] }));
}

function Image$1({ id, debug, image: { url, alt }, text }) {
    return (jsxs(Figure, Object.assign({ "data-id": id, debug: debug }, { children: [jsx("img", { src: url, alt: alt }), jsx("figcaption", { children: text })] })));
}

function Image({ image = {} }) {
    if (Object.prototype.hasOwnProperty.call(image, 'url') && image.url) {
        return jsx("img", { src: image.url, alt: image.alt });
    }
    return null;
}

function Information$1({ debug, heading, id, text, warning, printonly, printhide, }) {
    return (jsx(TextBlock, Object.assign({ warning: warning, debug: debug, "data-id": id, printonly: printonly, printhide: printhide }, { children: jsxs(Information$2, { children: [jsx(ErrorIcon, { triangleFill: 'black' }), jsx(Html, { text: heading, h2: true }), jsx(Html, { text: text })] }) })));
}

function Missing({ type = "No type" }) {
    return jsxs("div", { children: ["Block missing: ", type] });
}

function Number$1({ heading, property, errors, disabled, currentValue, placeholder, autocomplete, setData, maximum, minimum, step = 1, unit, }) {
    var _a, _b, _c;
    const handleChange = (e) => {
    ...
    if ((_a = errors === null || errors === void 0 ? void 0 : errors.validation) === null || _a === void 0 ? void 0 : _a.error)
        describedBy.push(`${property}.error`);
    return (jsxs("div", { children: [jsx(NumberInput, { "aria-label": heading, "aria-invalid": (_b = errors === null || errors === void 0 ? void 0 : errors.validation) === null || _b === void 0 ? void 0 : _b.error, "aria-describedby": describedBy.length > 0 ? describedBy.join(' ') : undefined, disabled: disabled, id: property, max: maximum, min: minimum, onChange: handleChange, autoComplete: autocomplete, placeholder: placeholder, step: step, type: "number", unit: unit, validation: errors === null || errors === void 0 ? void 0 : errors.validation, value: currentValue }), unit ? jsx(Html, { inline: true, id: `${property}.unit`, text: unit }) : null, ((_c = errors === null || errors === void 0 ? void 0 : errors.validation) === null || _c === void 0 ? void 0 : _c.error) && (jsxs(ErrorMessage, Object.assign({ role: "alert", id: `${property}.error` }, { children: [jsx(ErrorIcon, {}), " ", errors.validation.message] })))] }));
}
...
function RadioInput({ id, value, checked, debug, disabled, heading, image, messages, onChange, text = '', name }) {
    const message = messages === null || messages === void 0 ? void 0 : messages[0];
    return (jsxs("div", { children: [jsx(Radio$1, { "aria-label": heading, type: "radio", name: name, id: id, value: value, checked: checked, disabled: disabled, onChange: onChange }), jsxs(Label, Object.assign({ htmlFor: id, warning: message === null || message === void 0 ? void 0 : message.warning, debug: debug }, { children: [jsxs("div", Object.assign({ "data-id": id }, { children: [jsxs("div", { children: [heading && !text ? heading : null, heading && text ? jsx(H3, { children: heading }) : null, message ? (jsx(SoftWarning, Object.assign({ warning: message.warning }, { children: message.message }))) : null] }), text ? jsx(Html, { text: text }) : null] })), image && image.url ? jsx("img", { src: image.url, alt: image.alt }) : null] }))] }));
}

function Radio({ property, heading, text, grid, options, disabled, clear, debug, setData, currentValue, data }) {
    const handleChange = (property, value) => () => {
        if (clear === null || clear === void 0 ? void 0 : clear.length) {
    ...
    return (jsxs(Fieldset, { children: [jsx(SRLegend, { children: heading }), jsx(OptionWrapper, Object.assign({ grid: grid }, { children: options.map((option) => {
                    const isDisabled = disabled || option.disabled;
                    return (createElement(RadioInput, Object.assign({}, option, { data: data, debug: debug, key: `${property}:${option.value}`, name: property, "data-id": option.id, disabled: isDisabled, checked: currentValue === option.value, onChange: !isDisabled ? handleChange(property, option.value) : undefined })));
                }) }))] }));
}

function SelectOption({ id, value, heading, debug }) {
    return (jsxs("option", Object.assign({ value: value }, { children: [heading, debug ? ` (${id})` : null] })));
}

const NULL_VALUE = '({[NULL]})';
function Select({ setData, property, options, defaultOption, heading, currentValue, autocomplete, debug, data }) {
    const handleChange = (e) => {
        ...
        if (intValue && !isNaN(intValue)) {
            const optionExists = options.find(({ value: optionValue }) => optionValue === intValue);
    ...
    return (jsx(SelectWrapper, { children: jsxs("select", Object.assign({ "aria-label": heading, value: currentValue, onChange: handleChange, autoComplete: autocomplete }, { children: [jsx("option", Object.assign({ value: NULL_VALUE }, { children: placeholder })), options.map(option => (createElement(SelectOption, Object.assign({ data: data, debug: debug }, option, { key: option.value }))))] })) }));
}

const Signature$1 = injectStyles(styled.div `
  width: 100%;
  max-width: 300px;
...
function Signature() {
    return (jsx(PrintForm, { children: jsx(Signature$1, { children: jsx("div", { children: "Dato og signatur" }) }) }));
}

const Details = injectStyles(styled.details `
  margin-top: 40px;
  h3,
  h4 {
    font-size: 1em;
...
`);
const Summary$1 = injectStyles(styled.summary `
  cursor: pointer;
  color: ${({ styles }) => styles.color.light};
  text-decoration: underline;
`);
...
function SummaryDetails({ summary, details }) {
    return (jsxs(Details, { children: [jsx(Summary$1, { children: summary }), jsx(Html, { text: details })] }));
}

const calculateSum = (data, values, operations = [], minimum) => {
    var _a;
    let sum = values.reduce((accumulator, cur, currentIndex) => {
        const getValue = (defaultValue) => typeof cur === 'string' ? get$2(data, cur, defaultValue) : cur;
        ...
        return accumulator + get$2(data, cur, 0);
    }, ['+', '-'].indexOf((_a = operations === null || operations === void 0 ? void 0 : operations[0]) !== null && _a !== void 0 ? _a : '+') > -1 ? 0 : 1);
    if (typeof minimum === 'number') {
        sum = Math.max(Number(sum), minimum);
    }
    return sum;
};
function Sum$2({ data, values, operations, minimum, groupedSimple, final, heading, unit, summary, details }) {
    const sum = calculateSum(data, values, operations, minimum);
    return (jsxs(StyledSum, Object.assign({ groupedSimple: groupedSimple, final: final }, { children: [jsxs("div", { children: [heading, jsxs("span", { children: [sum, " ", unit ? jsx(Html, { inline: true, text: unit }) : null] })] }), summary && jsx(SummaryDetails, { summary: summary, details: details })] })));
}
...
function TableHeader(_a) {
    var props = __rest(_a, ["simple"]);
    return jsx("th", Object.assign({}, props));
}
/* eslint-disable react/no-array-index-key */
function Table({ cells, debug, simple }) {
    return (jsx(TableContainer, Object.assign({ simple: simple }, { children: jsx("table", Object.assign({ "data-debug": debug }, { children: jsx("tbody", { children: cells.map((row, rowKey) => (jsx("tr", { children: row.map((cell, cellKey) => {
                        const cellProps = Object.assign(Object.assign({}, pick$1(cell, ['colSpan', 'rowSpan'])), { 'data-inactive': Boolean(cell.inactive) });
                        const Cell = cell.type === 'Heading' ? TableHeader : TableCell;
                        return (createElement(Cell, Object.assign({ "data-id": cell.id }, cellProps, { key: cellKey, simple: simple }),
                            jsx(Html, { text: cell.text })));
                    }) }, rowKey))) }) })) })));
}

function Text({ debug, heading, id, text, warning = false, printonly = false, printhide = false, }) {
    return (jsxs(TextBlock, Object.assign({ warning: warning, debug: debug, "data-id": id, printonly: printonly, printhide: printhide }, { children: [jsx(Html, { text: heading, h2: true }), ";", jsx(Html, { text: text })] })));
}
...
function Textarea(props) {
    ...
    return (jsxs("div", { children: [jsx(Textarea$1, { "aria-label": heading, "aria-describedby": information ? `${property}.information` : undefined, placeholder: placeholder, value: currentValue, onChange: handleChange, autoComplete: autocomplete }), information && (jsxs(Information$2, Object.assign({ id: `${property}.information` }, { children: [jsx(ErrorIcon, { triangleFill: 'black' }), jsx(Html, { text: information })] })))] }));
}
...
function PureBlock(props) {
    const SpecificBlock$1 = getBlock(props.type);
    if (props.type === "ErrorOk") {
        return (jsx("div", { children: props.children.map((block) => (jsx(ConnectedBlock, Object.assign({ "data-id": props.id, simple: props.simple, setPage: props.setPage, errorPages: props.errorPages, pages: props.pages }, block), block.id))) }));
    }
    if (props.type === "Group") {
        return (jsxs(SpecificBlock, Object.assign({ "data-id": props.id, debug: props.debug }, { children: [jsx(Html, { text: props.heading, h2: true }), jsx(Html, { text: props.text }), props.children.map((block) => (jsx(ConnectedBlock, Object.assign({ grouped: true, "data-id": props.id, simple: props.simple, setPage: props.setPage, errorPages: props.errorPages, pages: props.pages }, block), block.id))), props.summary && (jsx(SummaryDetails, { summary: props.summary, details: props.details }))] })));
    }
    if (props.type === "Error") {
        return (jsxs(StyledErrorBlock, Object.assign({ role: "alert", "data-id": props.id, debug: props.debug }, { children: [jsx(Html, { text: props.heading, h2: true }), jsx(Html, { text: props.text }), props.children.map((block) => (jsx(ConnectedBlock, Object.assign({ grouped: true, "data-id": props.id, simple: props.simple, setPage: props.setPage, errorPages: props.errorPages, pages: props.pages }, block), block.id)))] })));
    }
    if (!SpecificBlock$1) {
        return jsx(Missing, { type: props.type });
    }
    if (props.type === "Image" ||
        props.type === "Text" ||
        ...
        return (jsx(SpecificBlock$1, Object.assign({ grouped: props.grouped, groupedSimple: props.simple, debug: props.debug, "data-id": props.id }, props)));
    }
    return (jsx(SpecificBlock, Object.assign({ "data-id": props.id, id: props.property, debug: props.debug, grouped: props.grouped, groupedSimple: props.simple, disabled: props.disabled, type: props.type }, { children: jsxs("div", { children: [jsx(Html, { text: props.heading, h2: true }), jsx(Html, { text: props.text }), jsx(Image, { image: props.image }), repeatQuestion(props.image, props.text) ? (jsx(Html, { text: props.heading, h3: true })) : (""), jsx(SpecificBlock$1, Object.assign({ debug: props.debug }, Object.assign(Object.assign({}, props), { validation: props.currentValue && props.validator
                        ? {
                            error: !new RegExp(props.validator.pattern).test(props.currentValue),
                            message: props.validator.error,
                        }
                        : {} }))), props.disabled && (jsxs(ErrorMessage, Object.assign({ role: "alert" }, { children: [jsx(ErrorIcon, {}), " ", props.errorDescription] }))), props.summary && (jsx(SummaryDetails, { summary: props.summary, details: props.details }))] }) })));
}
PureBlock.propTypes = {
...
const ConnectedBlock = connect((state, props) => ({
    data: state[NAME],
...
}), (dispatch) => bindActionCreators({ setData }, dispatch))(PureBlock);

function Navigation({ nextPageIsResult, hasNext = false, hasPrevious = false, nextPage = () => { }, previousPage = () => { }, }) {
    return (jsxs(Nav$1, { children: [hasPrevious ? (jsx(NavButton$1, Object.assign({ type: "button", onClick: previousPage }, { children: "Forrige" }))) : null, hasNext ? (jsx(NavButton$1, Object.assign({ type: "button", next: true, onClick: nextPage }, { children: nextPageIsResult ? "Vis resultat" : "Neste" }))) : null] }));
}

function Page({ children = [], debug = false, details = "", firstPage, lastPage, heading = "Missing page heading", lead = "", nextPage, nextPageIsResult, pageid, previousPage, summary = "", }) {
    return (jsxs(Main, Object.assign({ debug: debug, "data-id": pageid, id: "main" }, { children: [jsx(H1, { children: heading }), jsx(Lead, { children: jsx(Html, { text: lead }) }), summary && jsx(SummaryDetails, { summary: summary, details: details }), children.map((block, index) => (jsx(ConnectedBlock, Object.assign({}, block), (hasProperty(block, 'id') && block.id) || index))), (!firstPage || !lastPage) && (jsx(Navigation, { nextPageIsResult: nextPageIsResult, hasPrevious: !firstPage, previousPage: previousPage, hasNext: !lastPage, nextPage: nextPage }))] })));
}
...
function hasSoftError(page) {
    ...
    return page.children.reduce((result, node) => result ||
        isOfType(node, ["Error"]) ||
        (hasProperty(node, "children") && hasSoftError(node)), false);
}
...
function ExportData({ data }) {
    ...
    return (jsx("div", { children: jsx(MainButton, Object.assign({ "aria-live": copied ? "assertive" : undefined, copied: copied, onClick: copyToClipboard }, { children: buttonText })) }));
}
const mapStateToProps$2 = (state, { exporter }) => ({
    data: exporter(getData(state)),
});
var ExportData$1 = connect(mapStateToProps$2)(ExportData);

const PageSummary$1 = injectStyles(styled.div `
  padding: 20px 0 20px;
  margin: 0 0 10px;
  background: white;
  line-height: 2;
...
`);
const NodeSummary$1 = injectStyles(styled.div `
  margin-bottom: 30px;
  h3 {
    font-size: 18px;
    font-weight: 300;
...
`);
const Value$1 = injectStyles(styled.div `
...
function Value({ value, node }) {
    ...
    if (!node.optional && [null, undefined, ""].includes(value)) {
        return jsx(Value$1, Object.assign({ missing: true }, { children: "* M\u00E5 fylles ut" }));
    }
    if (node.options) {
        let values;
        if (typeof value === "object") {
        ...
        values = values.map((val) => {
            const option = (node.options || []).find(({ value: optionValue }) => optionValue === val) || {};
        ...
        if (!node.optional && !values.length) {
            ...
            return jsx(Value$1, { children: "Ingen valgt" });
        }
        const commaSeparated = values.slice(0, -1);
        const lastOne = values.slice(-1)[0];
        // If only one value selected
        if (!commaSeparated.length) {
            return (jsx(Value$1, { children: jsx(Html, { inline: true, text: lastOne }) }));
        }
        // If more than one selected
        return (jsxs(Value$1, { children: [commaSeparated.join(", "), " og ", lastOne, "."] }));
    }
    return (jsxs(Value$1, { children: [value, " ", node.unit ? jsx(Html, { inline: true, text: node.unit }) : null] }));
}
...
function Sum$1({ node, node: { allMandatory, currentValue = {}, errors } }) {
    const matchingItems = numOfMatchingItemsInObject(currentValue, true);
    return (jsx("div", { children: jsxs(SpecificBlock, Object.assign({ grouped: true, smallMarginTop: true, error: allMandatory && errors.required }, { children: [jsxs("p", { children: [matchingItems === 0 && (jsx("strong", { children: "Du har ikke valgt noen alternativer, men for " })), matchingItems > 0 && (jsxs("strong", { children: ["Du har ", allMandatory && errors.required ? "kun" : "", " valgt", " ", numOfMatchingItemsInObject(currentValue, true), " av ", " ", node.options.length, " alternativer."] }))] }), node.options.map((item) => (jsxs("div", { children: [jsx(CheckboxResultDisplay, { type: "checkbox", id: `${node.id}-${item.value}`, checked: currentValue[item.value] || false, readOnly: true }), jsx(Label, Object.assign({ htmlFor: `${node.id}-${item.value}` }, { children: jsx("div", { children: item.heading }) }))] }, `${node.id}-${item.value}`)))] })) }));
}
const ConnectedSum$1 = connect((state) => ({
    data: state[NAME],
}))(Sum$1);
Sum$1.propTypes = {
...
function FetchOrgSummary({ value: { address = "", name = "", orgid = "", postcode = "", postplace = "", validApprovalAreas } = {}, }) {
    if (!orgid) {
        return jsx(Value$1, Object.assign({ missing: true }, { children: "* M\u00E5 fylles ut" }));
    }
    return (jsxs(DL, { children: [jsx("dd", { children: orgid }), jsx("dt", { children: "Firmaets navn" }), jsx("dd", { children: name }), jsx("dt", { children: "Adresse" }), jsx("dd", { children: address }), jsx("dt", { children: "Postnummer- og sted" }), jsxs("dd", { children: [postcode, " ", postplace] }), jsx(ApprovalAreas, { areas: validApprovalAreas, title: true })] }));
}

function hasAllValues(values, data) {
    return values.reduce((acc, currentValue) => get$2(data, currentValue) !== undefined && acc, true);
}
function Sum({ node, data }) {
    ...
    if (!hasAllValues(values, data)) {
        return jsx(Value$1, Object.assign({ missing: true }, { children: "* Mangler verdi for utregning" }));
    }
    const sum = values.reduce((accumulator, currentValue, currentIndex) => {
        if (operations && operations[currentIndex] === "-") {
            return Math.max(accumulator - get$2(data, currentValue), minimum);
        }
        else if (operations && operations[currentIndex] === "*") {
            return Math.max(accumulator * get$2(data, currentValue), minimum);
        }
    ...
    return jsx(Value$1, { children: sum });
}
...
function SoftError({ children }) {
    return (jsx(NodeSummary$1, { children: children.map((child) => {
            const heading = !isOfType(child, ['Result']) &&
                ((hasProperty(child, 'heading') && child.heading) ||
                    (hasProperty(child, 'text') && child.text));
            if (heading) {
                return (jsxs(ErrorMessage, Object.assign({ role: "alert" }, { children: [jsx(ErrorIcon, {}), " ", jsx(Html, { text: heading, inline: true })] })));
            }
            return null;
        }) }));
}
...
function Information({ text }) {
    if (text) {
        return (jsx(NodeSummary$1, { children: jsxs(Information$2, { children: [jsx(ErrorIcon, { triangleFill: "black" }), jsx(Html, { text: text })] }) }));
    }
    return jsx("p", { children: "TEXT MISSING" });
}

const ignoreNodes = ["Text", "Image"];
const overrideValueSummary = { FetchOrg: FetchOrgSummary, Sum: ConnectedSum, Checkbox: ConnectedSum$1 };
function NodeSummary({ node }) {
    if (isOfType(node, ["Error"])) {
        return jsx(SoftError, { children: node.children });
    }
    if (isOfType(node, ["Information"])) {
        return jsx(Information, { text: node.text });
    }
    if (isOfType(node, ["Group", "ErrorOk"])) {
        return (jsx("div", { children: node.children.map((child) => (jsx(NodeSummary, { value: child.currentValue, node: child }, child.id))) }));
    }
    if (ignoreNodes.includes(node.type) || !hasProperty(node, 'property')) {
        return null;
    }
    const ValueSummary = get$2(overrideValueSummary, node.type, Value);
    const errors = get$2(node, 'errors');
    const errorDescription = get$2(node, 'errorDescription');
    return (jsxs(NodeSummary$1, { children: [jsx(Html, { text: node.heading, h3: true }), jsx(ValueSummary, { value: get$2(node, 'currentValue'), node: node }), errors.validation.error ? (jsxs(ErrorMessage, Object.assign({ role: "alert" }, { children: [jsx(ErrorIcon, {}), errors.validation.message] }))) : null, errors.disabled.length ? (jsxs(ErrorMessage, Object.assign({ role: "alert" }, { children: [jsx(ErrorIcon, {}), " ", errorDescription] }))) : null] }));
}
...
function PageSummary({ children = [], error, goToPage, heading }) {
    return (jsxs(PageSummary$1, Object.assign({ error: error }, { children: [jsxs(H2, { children: [heading, " ", jsx(SmallButton, Object.assign({ type: "button", onClick: goToPage }, { children: "Endre svar" }))] }), children.map((node, index) => (jsx(NodeSummary, { node: node }, hasProperty(node, 'id') ? node.id : index)))] })));
}

function Summary({ errorPages, setPage, pages = [] }) {
    return (jsx("div", { children: pages
            .filter(({ type }) => type === "Page")
            .map((page) => (hasProperty(page, 'id') ? jsx(PageSummary, Object.assign({ error: !!errorPages.find(({ id }) => page.id === id) ||
                hasSoftError(page), goToPage: () => setPage(page.id) }, page), page.id) : null)) }));
}

var Spinner = injectStyles(styled.div `
  display: inline-flex;
  width: 1em;
  height: 1em;
...
function PDFButton({ localStorageKey, pageHeading, pdfServiceUrl, }) {
    ...
    return (jsxs("div", { children: [jsxs(MainButton, Object.assign({ type: "button", onClick: handleClick, disabled: isDownloading }, { children: [jsx("span", { children: isDownloading ? 'Lager PDF...' : 'Lagre som PDF' }, "text"), jsx(Spinner, { hidden: !isDownloading }, "spinner")] })), errorMessage && jsx("p", { children: "Noe gikk galt. Pr\u00F8v igjen." })] }));
}

function Result(props) {
    const { children = [], debug, errorPages, exporter, heading, lead, exports, pageid, schema, setPage, summaryTitle, title, wizard: { meta: { pdfServiceUrl, localStorageKey }, }, } = props;
    ...
    return (jsxs(Main, Object.assign({ result: true, debug: debug, "data-id": pageid, id: "main" }, { children: [jsx(H1, Object.assign({ result: true }, { children: resultHeading })), jsx(Lead, Object.assign({ result: true }, { children: jsx(Html, { text: resultLead }) })), children.map((_a) => {
                var block = __rest(_a, ["heading"]);
                return (createElement(ConnectedBlock, Object.assign({}, block, { key: block.id, errorPages: errorPages, setPage: setPage, pages: schema })));
            }), jsxs(SpecificBlock, { children: [summaryTitle ? jsx(H1, Object.assign({ small: true }, { children: summaryTitle })) : jsx(H1, Object.assign({ small: true }, { children: title })), jsx(Summary, { errorPages: errorPages, setPage: setPage, pages: schema })] }), !incomplete && exporter ? (jsxs(TextBlock, { children: [jsx(H2, { children: "Takk for at du gjennomf\u00F8rte veiviseren!" }), jsx(Lead, { children: "Du kan kopiere svarene dine, eller skrive ut resultatsiden." })] })) : null, jsxs(Export, Object.assign({ exporter: exporter }, { children: [!incomplete && exporter && (exports === null || exports === void 0 ? void 0 : exports[exporter]) ? (jsx(ExportData$1, { exporter: exports[exporter] })) : null, pdfServiceUrl && localStorageKey ? (jsx(PDFButton, { pdfServiceUrl: pdfServiceUrl, localStorageKey: localStorageKey, pageHeading: resultHeading })) : (jsx(MainButton, Object.assign({ type: "button", onClick: printPage }, { children: "Skriv ut" })))] }))] })));
}
const mapStateToProps$1 = (state, { wizard }) => ({
    errorPages: getErrorPages(wizard.schema, state),
});
var Result$1 = connect(mapStateToProps$1)(Result);

const SkipLink$1 = injectStyles(styled.a `
  position: absolute;
  width: 1px;
  height: 1px;
  padding: 0;
...
const SkipLink = () => (jsx(SkipLink$1, Object.assign({ href: "#main" }, { children: "G\u00E5 til hovedinnhold" })));
...
function assertProperties(object, path, properties) {
    ...
    (properties || []).forEach((property) => {
        if (get$2(object, property, undefined) === undefined) {
            errors = [
                ...errors,
                {
                    path,
                    id: get$2(object, "id"),
                    error: `${get$2(object, "type")} is missing the '${property}' property`,
                },
            ];
        }
    });
        ...
        if (get$2(object, property, undefined) !== undefined) {
            errors = [
                ...errors,
                {
                    path,
                    id: get$2(object, "id"),
                    error: `${get$2(object, "type")} is using the '${property}' property. It's deprecated and will be removed.${use ? ` Use '${use}' instead` : ""}.`,
                },
            ];
        }
    });
...
function validateNode(node, path, ancestors) {
    ...
    if (ancestors.length && get$2(node, "type") === "Page") {
        errors = [
            ...errors,
            {
                path,
    ...
    logicalExpressionFields.forEach((field) => {
        ...
        try {
            parseExpression(get$2(node, field));
        }
        catch (e) {
            errors = [
                ...errors,
    ...
    if ((node.type === "Page" || node.type === "Group") && node.children) {
        node.children.forEach((child, index) => {
            errors = [
                ...errors,
                ...validateNode(child, [...path, "children", String(index)], [...ancestors, node]),
            ];
        });
    }
    ...
    if (node.type === "Branch" && node.branches) {
        node.branches.forEach((branch, branchIndex) => {
            errors = [
                ...errors,
                ...assertProperties(branch, [...path, "branches", String(branchIndex)], ["test", "children"]),
            ];
    ...
    if ((node.type === "Radio" || node.type === "Checkbox") && node.options) {
        node.options.forEach((option, index) => {
            errors = [
                ...errors,
                ...validateNode(option, [...path, "options", String(index)], [...ancestors, node]),
            ];
        });
    }
    // Check image
    if (node.type === "Image" && node.image) {
        ...
        errors = [
            ...errors,
            ...assertProperties(Object.assign(Object.assign({}, node.image), { type: "Image" }), [...path, "image"], ["url", "alt"]),
        ];
    }
    return errors;
}
...
function validateRootBranch(branchNode, path) {
    ...
    return [
        ...
        ...((_b = (_a = branchNode === null || branchNode === void 0 ? void 0 : branchNode.branches) === null || _a === void 0 ? void 0 : _a.reduce((errors, branch, index) => {
        ...
        ...((_d = (_c = branchNode === null || branchNode === void 0 ? void 0 : branchNode.branches) === null || _c === void 0 ? void 0 : _c.reduce((errors, node, branchIndex) => [
            ...errors,
            ...node === null || node === void 0 ? void 0 : node.children.reduce((childBranchErrors, childBranch, childIndex) => [
                ...childBranchErrors,
                ...validateNode(childBranch, [
                    ...path,
                    "branches",
                    String(branchIndex),
                    "children",
...
function Wizard({ data = {
    [NAME]: {}
}, debug = false, warningBeforeUnload = true, exports = {}, showIntro = () => { }, styles = {}, translations = {}, showResetModal: showResetModalDefault = true, schema, setData, tableOfContents, title, wizard }) {
    const [pageId, setPageId] = useState(data[NAME].page || wizard.schema[0].id);
    useState(false);
    const [showResetModal, setShowResetModal] = useState();
    const pageWrapper = useRef(null);
    // @todo Consider finding a more elegant way for scrolling..?
    const setPage = (page, property) => {
        if (property) {
            setImmediate(() => {
    ...
    const trackPage = (isInitialRender) => {
        ...
        if (isInitialRender) {
            ...
            const page = (schema || []).find((item) => hasProperty(item, 'id') && item.id === pageId);
    ...
    useEffect(() => {
        const _a = data[NAME] || {}, wizardData = __rest(_a, ["$computed"]);
        if (wizardData.page) {
...
${error}
    ...
    function getCurrentIndex() {
        return Math.max(0, schema.findIndex(({ id }) => id === getData(data).page));
    }
    ...
    const nextPageIsResult = schema[pageIndex + 1]
        ...
        : false;
    return (jsx(StyleProvider, Object.assign({ styles: styles }, { children: jsxs(Wizard$2, { children: [showResetModal && jsx(Modal$1, { showIntro: showIntro }), jsxs(Grid, { children: [jsx(SkipLink, {}), jsx(Nav, { page: page === null || page === void 0 ? void 0 : page.id, setPage: setPage, heading: title, tableOfContents: tableOfContents, showIntro: showIntro, translations: translations }), jsx(FocusWrapper, Object.assign({ tabIndex: "-1", ref: pageWrapper }, { children: page && isOfType(page, ["Result"]) ? (jsx(Result$1, Object.assign({}, page, { debug: debug, pageid: page.id, wizard: wizard, title: title, schema: schema, setPage: setPage, exports: exports }))) : (page && isOfType(page, ["Page", "Error", "ErrorOk"]) && (jsx(Page, Object.assign({ firstPage: firstPage, lastPage: lastPage, nextPage: nextPage, previousPage: previousPage, debug: debug, nextPageIsResult: nextPageIsResult, pageid: page.id }, page)))) }))] })] }) })));
}
...
var Wizard$1 = connect(mapStateToProps, (dispatch) => bindActionCreators({ setData }, dispatch))(Wizard);

export { index as Primitives, StyleProvider, Wizard$1 as Wizard, index$3 as dsl, index$2 as state, track, trackEvent };
